...
export default function SentScreen() {
  ...
  const allMails = useSelector((s) => s.mail.items || []);
  ...
  const sent = allMails.filter((m) => {
    const from = (m.from_email || m.from || '').toLowerCase();
    return userEmail ? from === userEmail.toLowerCase() : false;
  });
    ...
    return (
      ...
      >
        <View style={styles.left}>
          {item.to_avatar ? (
            <Avatar.Image size={52} source={{ uri: item.to_avatar }} />
          ) : (
    ...
    <View style={styles.container}>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={<View style={styles.empty}><Text style={styles.emptyText}>No sent messages</Text></View>}
        />
      )}
      ...
      <View style={styles.bottomBar}>
        <TouchableOpacity style={styles.tabItem}>
          <IconButton icon="inbox" color="#94a3b8" size={22} />
          <Text style={styles.tabLabel}>Inbox</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.tabItem}>
          <IconButton icon="send" color="#2f80ed" size={22} />
          <Text style={[styles.tabLabel, { color: '#2f80ed' }]}>Sent</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.tabItem}>
